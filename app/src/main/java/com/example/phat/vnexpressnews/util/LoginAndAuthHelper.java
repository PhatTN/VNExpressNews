 ...
 * also getting the auth token for the necessary scopes. The life of this object is tied
 * to an Activity. Do not attempt to share it across Activities, as unhappiness will result.
 */
public class LoginAndAuthHelper
    ...
    private static final int REQUEST_AUTHENTICATION = 100;
    ...
    // When sign-in fail, we will show the UI only once and set this flag to false.
    ...
    // The Activity this object is bound to (we use a weak reference to avoid context leaks)
    ...
    public interface Callbacks {
        void onPlushInfoLoaded(String accountName);
    ...
    public void onConnectionFailed(ConnectionResult connectionResult) {
        ...
        if (connectionResult.hasResolution()) {
            if (sCanShowSignInUi) {
                LOGD(TAG, "onConnectionFailed, with resolution. Attemping to resolve.");
    ...
    public void onResult(People.LoadPeopleResult loadPeopleResult) {
        ...
        if (loadPeopleResult.getStatus().isSuccess()) {
            ...
            if (personBuffer != null && personBuffer.getCount() > 0) {
                ...
                AccountUtils.setPlusImageUrl(mAppContext, mAccountName, currentUser.getDisplayName());
                ...
                if (null != (callbacks = mCallbacksRef.get())) {
                    callbacks.onPlushInfoLoaded(mAccountName);
                }
            } else {
            ...
            activity.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (mStarted) {
                        ...
                        LOGE(TAG, "Activity not started, so not show auth recovery flow.");
                    }
                }
            });
        }
