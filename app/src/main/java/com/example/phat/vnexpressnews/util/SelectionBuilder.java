...
/**
 * Helper for build selection clauses for {@link android.database.sqlite.SQLiteDatabase}.
 * Each appended clause is combined using {@code AND}. This class is <em>not</em>
 * thread safe.
 */
public class SelectionBuilder {
    ...
    public SelectionBuilder mapToTable(String column, String table) {
        mProjectMap.put(column, table + "." + column);
        return this;
    }

    public SelectionBuilder map(String fromColumn, String toClause) {
        mProjectMap.put(fromColumn, toClause + " AS " + fromColumn);
        return this;
    }
    ...
    private void mapColumns(String[] columns) {
        for (int i = 0; i < columns.length; i++) {
            final String target = mProjectMap.get(columns[i]);
            if (target != null) {
                columns[i] = target;
            }
        }
    ...
    public String toString() {
        return "SelectionBuilder[table=" + mTable + ", selection=" +  getSelection()
                ...
                + ", projectionMap=" + mProjectMap + "]";
    }

    /**
     * Execute query using the current internal state as {@code HERE} clause.
     */
